...
func Send(c *gin.Context) {
	...
	rsp.Msg = "参数错误"
	var args formBind.SendCode
	if c.Bind(&args) == nil {
		//var gRsp gateway.CallRsp
		//opt := &gateway.Call{
		//	Type:   gateway.CallType_User,
		//	Caller: gateway.CallerType_SendCode,
		//	Opt: &gateway.Options{
		...
		gRsp, err := callHelper.NewCall(c.ClientIP(), "").Call(gateway.CallType_User, gateway.CallerType_SendCode).Do(opt)
		if err != nil {
			rsp.Data = err.Error()
		} else {
			if gRsp.Status == true {
